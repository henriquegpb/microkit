 ...
 * through the shadcn CLI: this repository's registry publishes interactions,
 * and site chrome does not belong in it. Kept as close to upstream as possible
 * so it can be diffed against a newer version — the only edits are two ternary
 * statements rewritten as `if`/`else` to satisfy `no-unused-expressions`. The
 * site drives it entirely through props from `home-background.tsx`.
 *
 * Not a catalog interaction: it is not in `content/interactions`, it ships no
 * CSS/Tailwind twin, and nothing here is copy-and-paste material.
 */
