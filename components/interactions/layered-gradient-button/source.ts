...
.layered-gradient-button > span:first-child { position: relative; z-index: 2; background: linear-gradient(to right, light-dark(#14181e,#f0f0f0), light-dark(#7c8695,#464646) 48%, light-dark(#14181e,#f0f0f0)); background-clip: text; color: transparent; font-size: 18px; font-weight: 500; transition: transform .3s ease; }
.layered-gradient-button-surface { position: absolute; z-index: 1; inset: 0; border: 1px solid light-dark(#14181e33,#f0f0f033); border-radius: inherit; background: radial-gradient(circle at 50% 100%, light-dark(#ffffff,#111), light-dark(#e9ecf1,#252525)); transition: transform .3s ease; }
.layered-gradient-button-haze { position: absolute; z-index: 0; top: 4px; width: 104%; height: 98%; border-radius: inherit; background: light-dark(#14181e80,#f0f0f080); opacity: 0; transform: scale(.8); transition: opacity .6s ease, transform .3s ease; }
