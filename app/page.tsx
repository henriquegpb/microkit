...
export default function Home() {
  ...
  useEffect(() => {
    const openComponent = (event: MouseEvent) => {
      const target = event.target as HTMLElement;
      const card = target.closest<HTMLElement>(".interaction-card");
      const demo = target.closest<HTMLElement>(".demo");
      if (!card || target.closest(".favorite") || (demo && target !== demo)) return;
      const index = [...document.querySelectorAll(".gallery-grid .interaction-card")].indexOf(card);
      const item = filtered[index];
      if (item) window.location.assign(`/components/${item.id}`);
    };
    document.addEventListener("click", openComponent);
    return () => document.removeEventListener("click", openComponent);
  }, [filtered]);
