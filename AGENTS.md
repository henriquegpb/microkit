...
- `tailwindCode` in `source.ts` is also the canonical source for the shadcn registry, which distributes the TypeScript + Tailwind variant. After changing it for an interaction published in `registry.json`, run `npm run registry:build` and commit the regenerated `registry/microkit/*.tsx` and `public/r/*.json` in the same change. Never hand-edit those generated files.
